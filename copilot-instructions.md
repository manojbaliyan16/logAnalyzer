...
### Path Handling
- **Cross-platform support**: Use `os.path.join()` or `pathlib.Path` for all path operations
- Config paths: Use `os.path.join('configs', 'feature_config.json')`
...
- **Migration Note**: Replace all hardcoded `\\` with `os.sep` or `pathlib.Path`

### File Type Handling
- **`.dlt` files**: DLT (Diagnostic Log and Trace) format - automotive standard log format
- **`.txt` files**: Plain text logs - direct processable
- **Coredump/Backtraces**: Core dump files for crash analysis
- `.pro` files: Direct processable logs (legacy)
- `.bin` files: Require decoding with TRC files using decoders
...
python EM_Analyzer_main.py  # or python3 on Linux
```
...
### Processing New Log Types
The system automatically detects and processes:
- **DLT files** → Converted to text via `DLTLogHandler` → Saved to `DLT_LOGS/`
- **TXT files** → Encoding normalized via `TextLogHandler` → Saved to `TEXT_LOGS/`
- **Coredumps** → Backtrace extracted via `CoredumpHandler` → Saved to `COREDUMP_LOGS/`
...
- **External Tools** (in `exes/`): Platform-specific decoders
  - Windows: `Decoder.exe`, `ExtractTrcFiles.exe`, `geckodriver.exe`
  - Linux: Corresponding binaries or use platform-agnostic Python libraries
- **DLT Processing**: Consider `python-dlt` or `dlt-viewer` libraries for .dlt files
- **Coredump Analysis**: `gdb` (Linux), WinDbg (Windows), or crash analysis tools
- **Selenium/geckodriver**: Used for dashboard interactions (Firefox) - cross-platform
...
## Common Pitfalls
- **Cross-platform paths**: Always use `os.path.join()` or `pathlib.Path` - never hardcode `/` or `\\`
- **External tool dependency**: Ensure platform-specific decoders exist before running
...
- **New file types**: `.dlt`, `.txt`, and coredump files require specific parsers/handlers
- **Platform detection**: Use `platform.system()` to conditionally select decoders/tools
