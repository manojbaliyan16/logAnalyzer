...
class Ticket:
    ...
    def validateTicketAttachments(self,attachmentsPathList):
        status="INVALID"
        # Updated to support .dlt, .txt, and coredump files
        valid_extensions = [".bin", ".pro", ".txt", ".exv", ".zip",
                           ".dlt", ".core", ".dump", ".dmp", ".backtrace"]

        for path in attachmentsPathList:
            # Check for valid extensions or special downloadPipe case
            if any(path.endswith(ext) for ext in valid_extensions) or "downloadPipe" in path:
                status = "VALID"
                break
        return status  
