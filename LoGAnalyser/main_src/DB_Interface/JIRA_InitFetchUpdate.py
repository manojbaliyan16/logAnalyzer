...
def createTicketFolder(TicketNo,basefolderpath):
    ...
    ticketFolderPath=basefolderpath+ticketfoldername+"\\"
...
def downloadAttachments(reference,downloadPath):
    ticketFolderPath="GetLogs\\NCG3D-305200\\"
    ...
    for attachment in reference.fields.attachment:
        downloadfilepath=ticketFolderPath+attachment.filename
        if((downloadfilepath.endswith(".pro")) or (downloadfilepath.endswith(".bin")) or (downloadfilepath.endswith(".zip")) or (downloadfilepath.find("downloadPipe"))):
            data = attachment.get()
